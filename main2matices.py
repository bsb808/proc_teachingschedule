...
def copy_cells_with_formatting(source_file, destination_file, source_sheet_name, destination_sheet_name, source_range, destination_start_cell):
	...
	for row_index, row in enumerate(source_sheet[source_range]):
		for col_index, source_cell in enumerate(row):
			...
			destination_cell = destination_sheet.cell(row=destination_start_cell[0] + row_index, column=destination_start_cell[1] + col_index, value = source_cell.value)
...
# Example usage
source_file_path = './MAE_Master_Course_Plan_AY241.xlsx'
destination_file_path = 'matrix.xlsx'
